...
import { useStatistiquesMecanicienContext } from '../hooks/useStatistiquesMecanicienContext';
...
const Mecanicien = () => {
  ...
  const { statistiques, dispatch } =useStatistiquesMecanicienContext()
  useEffect(() => {
    const fetchTotalStatistic = async () => {
      const response = await fetch(API_BACKEND + '/api/mecanicien/statistique/' + user.id);
...
<div className="p-4 sm:ml-64">
      <div className="p-4 border-2 border-gray-200 border-solid rounded-lg dark:border-gray-700">
          ...
          <div className="grid grid-cols-1 gap-4 mb-4 md:grid-cols-1 lg:grid-cols-3 xl:grid-cols-3">
            ...
            <div className="flex flex-col items-center justify-center h-24 rounded bg-blue-400 dark:bg-blue-600">
              <GiAutoRepair className="text-4xl text-white mb-2" />
              <p className="text-2xl text-white">Total : {statistiques.totatReparation.termine + statistiques.totatReparation.en_cours}</p>
            </div>
          </div>
          {/* ----- end 1 ------ */}
          ...
          <div className="grid grid-cols-1 gap-4 mb-4 mt-14 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-2">
            <div className="flex items-center justify-center rounded bg-gray-50 h-96 dark:bg-gray-800">
              <Pie
                data={{
                  labels: statistiques.reparationMecanicien.map(data => data.label),
                  datasets: [
                    {
                      label: 'Count',
                      data: statistiques.reparationMecanicien.map(data => data.value),
                      backgroundColor: [
                        'rgba(255, 152, 0, 0.8)',
                        'rgba(76, 175, 80, 0.8)',
                        'rgba(43, 63, 229, 0.8)',
