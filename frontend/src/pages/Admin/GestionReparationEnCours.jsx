...
import { useReparationsEnCoursContext } from '../../hooks/useReparationsEnCoursContext'

const GestionReparationEnCours = () => {
    const {clients , dispatch:dispatchClients} = useClientsContext()
    const {reparations ,dispatch } = useReparationsEnCoursContext()
    ...
    useEffect(()=>{
        const fetchReparations = async ()=>{
            const response = await fetch(API_BACKEND +'/api/admin/reparations/2' , {
    ...
    const columns = [
    ...
    
        {
            ...
            cell : (row) => <div className='flex items-center justify-center'>
                ...
                <button  onClick={ ()=>{affecterReparationToMecanique(row.id , row.mecanique_id)}}  className=' text-green-700 hover:text-white border border-green-700 hover:bg-green-800 focus:outline-none font-medium rounded-lg text-xs px-1 py-0.5 text-center flex'>affecter <span><MdModeEdit className='ml-1' /></span> </button>
    ...
    const affecterReparationToMecanique = (id , id_mecanique)=>{

        console.log(id_mecanique);
        setIsModalAffectationOpen(true)
        setMecaniqueVoitureEmail(clients.filter((item)=> item.id === id_mecanique)[0].email )
        
        setIdMecanique(id_mecanique)
        setIdReparation(id)
        setError("")
    ...
    const handleAffectReparationMecaniqueSubmit = async(e)=>{
        ...
        const response = await fetch(API_BACKEND + '/api/admin/reparation/updateaffectation' , {
            method: 'POST',
            headers: {
...
export default GestionReparationEnCours